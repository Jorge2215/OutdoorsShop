...
### 2026-05-27T16:51:34.303-03:00 — Async order receipts

- Added a shared `ReceiptGenerationMessage` contract plus `IReceiptQueuePublisher`/`ReceiptQueuePublisher` so successful payment confirmations can enqueue receipt work onto `receipt-requests`.
- Chose `PaymentConfirmationFunction` as the producer because payment success is the actual confirmation event in this repo; the API order-create path still represents an unpaid order.
- Added `ReceiptGenerationFunction` to render deterministic HTML receipts into the private `order-receipts` container, reusing `IBlobStorageService` and deterministic blob naming (`orders/{orderId}/receipt.html`) for retry-safe uploads.
- Added `GET /api/v1/orders/{id}/receipt` via `IOrderService.GetReceiptAsync`, returning receipt availability plus a short-lived SAS URL when the blob exists.
- Validation passed with `dotnet build .\\OutdoorsShop.slnx --verbosity minimal`, `dotnet test .\\tests\\OutdoorsShop.Api.Tests\\OutdoorsShop.Api.Tests.csproj --verbosity minimal -m:1 /p:UseSharedCompilation=false`, and `dotnet test .\\tests\\OutdoorsShop.Functions.Tests\\OutdoorsShop.Functions.Tests.csproj --verbosity minimal -m:1 /p:UseSharedCompilation=false`.
