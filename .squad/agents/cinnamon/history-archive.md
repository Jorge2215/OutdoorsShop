# cinnamon — History Archive

## 2026-05-28T01:15:13Z — Archived snapshot from history.md
...
### 2026-05-27T21:23:27.855-03:00 — Async report export pipeline

- Added async report export requests with persisted ReportExportRequests records, queue publishing, blob-backed download metadata, and new admin endpoints under POST/GET /api/v1/reports/requests plus GET /api/v1/reports/requests/{id}/download.
- Extracted shared report generation into ReportFileService so existing synchronous /api/v1/reports/orders and /api/v1/reports/inventory still work while the new ReportExportFunction reuses the same CSV/Excel generation logic.
- Added EF Core migration AddReportExportRequests and integration/function tests covering request creation, status/download retrieval, and queue-triggered export processing.
- Validation: dotnet test .\\OutdoorsShop.slnx --verbosity minimal passed.

## Learnings

### 2026-05-27T22:00:07.784-03:00 — Async report export dev rollout

- The async report export path only works when the API and Functions share the same Azure SQL database and the same Storage account: the API persists `ReportExportRequests` and enqueues the work item, while the Function dequeues it and updates that same row with blob metadata.
- `20260528003127_AddReportExportRequests` must be applied explicitly before deploy; neither `src\\OutdoorsShop.Api\\Program.cs` nor `src\\OutdoorsShop.Functions\\Program.cs` runs EF migrations on startup.
- Dev config must provide a storage connection string with account-key SAS capability for the API download endpoint, plus `AzureWebJobsStorage` for the Function trigger. Keep the queue name at the default `report-export-requests` unless code is updated, because the trigger is hardcoded while the publisher setting is configurable.
