# Creta history archive — snapshot 2026-05-25T223903Z
...
### 2026-05-25T11:05:01.947-03:00 â€” Admin Product Catalog Live Validation
...
### 2026-05-24T19:59:32.340-03:00 â€” Admin Product Catalog Test Scenario Design
...
- **RBAC is always Area 1.** All 10 RBAC scenarios must be green before any other area is considered shippable. Pattern confirmed: no-token â†’ 401, Customer JWT â†’ 403, Admin JWT â†’ 2xx. Fresh tokens required for auth test runs (stale Customer token returns 401 not 403).
...
- **Image upload core (22/22)** already covered â€” only 4 incremental catalog-context scenarios added for IMG area.
...
### 2026-05-24T16:52:12.609-03:00 â€” Team update
- Cinnamon delivered admin user seed (admin@outdoorsshop.dev / Admin@123456) â€” Creta can now rerun all blocked image upload tests.

### 2026-05-24 â€” Image Upload Test Execution (Run 3 â€” Final)

**Admin credentials confirmed:** `admin@outdoorsshop.dev` / `Admin@123456` â€” returns JWT with `Administrator` role claim. Blocker from Run 2 resolved.

**All 22 tests: âœ… PASS (0 FAIL, 0 BLOCKED)**
...
- Blob naming uses UUID scheme (`products/{productId}/{uuid}.{ext}`) â€” special chars in original filename are discarded, no injection risk.
...
- Product existence check returns 404 with `{"message":"Product {id} not found."}` â€” good API hygiene.
- Public access confirmed: blobs are accessible anonymously (no SAS needed) â€” container is `PublicAccessType.Blob` as designed.
...
**Verdict: âœ… PASS (22/22)**

### 2026-05-24 â€” Image Upload Test Execution (Run 2)

**Endpoint:** `POST /api/v1/products/{id}/image` â€” confirmed DEPLOYED as of 2026-05-24T16:52:12.609-03:00.

**Tests run: 3 / 17 pending**
- A-01: âœ… PASS â€” 401 without token (auth guard fires before any upload logic)
- A-02: âœ… PASS â€” 403 with Customer JWT (role guard working; token expiry pitfall: tokens expire in ~90 min, stale token returns 401 not 403)
- C-01: âœ… PASS â€” CORS preflight 204 from SWA origin, all headers correct

**Blocked: 14 / 17** â€” No admin user exists in the database.
- `Program.cs` seeds roles (`Administrator`, `Customer`) but does NOT seed an admin user account.
- 7 credential combinations attempted â€” all 401. No known admin password documented anywhere.
...
- Access tokens expire in ~90 minutes (previously stated as 15 min â€” actual observed TTL is closer to 90 min based on `exp` claim difference).
...
### 2026-05-24 â€” Image Upload Test Plan (image-upload-test-plan.md)

**Endpoint status:** `POST /api/v1/products/{id}/image` â†’ **404 NOT DEPLOYED** as of 2026-05-24T16:52:12.609-03:00.

**Tests run (infrastructure):**
- PRE-01: Health â†’ âœ… PASS
- PRE-02: Product 1 exists â†’ âœ… PASS (16 products, Alpine Base Camp Tent 4P at ID=1)
- PRE-03: Existing imageUrl publicly accessible â†’ âœ… PASS (Unsplash CDN, 200, image/jpeg)
- C-01: CORS OPTIONS from SWA origin â†’ âœ… PASS (204, correct CORS headers including `POST` in Allow-Methods)

**Tests pending (17 functional tests):** H-01..05, A-01..03, V-01..05, E-01..04 â€” all blocked on Cinnamon deploying the upload action in `ProductsController`.

**Key findings written to decisions inbox:**
1. No default admin user seeded â€” DB-level role assignment needed for admin JWT in tests
2. `BlobStorageService.UploadAsync` uses `PublicAccessType.None` â€” blobs won't be publicly accessible without SAS; container must use `PublicAccessType.Blob` for product images
3. CORS middleware responds to OPTIONS for 404 paths â€” CORS is verifiable before endpoint exists
4. Old blob cleanup is critical to test on re-upload (E-03)

**Skill added:** `.squad/skills/blob-image-upload-testing/SKILL.md` â€” minimal test image generation, PowerShell multipart upload helpers, CORS preflight helper, blob naming strategies.

### 2026-05-24 â€” Full E2E Journey Test (live endpoints)
...
- `POST /api/v1/auth/register` body: `{name, email, password, confirmPassword}` â€” NOT `{firstName, lastName}`. Combined `name` field, `confirmPassword` required.
- `POST /api/v1/auth/login` returns `{accessToken, ...}` â€” use `login.accessToken`.
- Refresh token is in `HttpOnly` cookie `refreshToken`, not in the JSON body.

**Cart is fully client-side:**
- ADR-004 confirmed: no server-side cart endpoints exist. `GET /api/v1/cart` â†’ 404 by design.
- Cart â†’ checkout journey requires Playwright browser tests (Zustand + localStorage).
...
- Wrong password login returns 401 (not 400 â€” confirmed via Invoke-WebRequest).

**Production blocker â€” missing roles:**
- `POST /api/v1/auth/register` â†’ 500 `"Role CUSTOMER does not exist."`
...
- `GET https://func-outdoors-dev.azurewebsites.net/api/health` â†’ 200 `{"status":"ok"}`

**SWA frontend serving:**
- `https://wonderful-plant-0a1ca5f0f.7.azurestaticapps.net` â†’ 200 OK

### 2026-05-24 â€” Azure Functions live test results
...
### 2026-05-23 â€” Comprehensive xUnit Test Suite (Controllers + Azure Functions)

**Moq patterns discovered:**

- `UserManager<T>` requires 9 constructor args â€” mock `IUserStore<T>` + 8 nulls. Wrap in `Mock<UserManager<T>>` to `.Setup()` methods. Same pattern for `SignInManager<T>` (7 args).
- `ClaimsPrincipal` for controller unit tests: use `new ClaimsIdentity(claims, "Test")` â€” the `"Test"` authenticationType is required for `Identity.IsAuthenticated` to return true.
...
- `HasData()` seed in `OnModelCreating` is NOT seeded into InMemory databases â€” must insert manually in tests.
- Use `Guid.NewGuid().ToString()` as database name to ensure isolation between tests.

**EF Core 10.0 known issue â€” WebApplicationFactory multi-provider conflict:**

- When `WebApplicationFactory` replaces `DbContextOptions<AppDbContext>` (SqlServer â†’ InMemory), EF Core 10.0 throws: `"Only a single database provider can be registered in a service provider."` even after `RemoveAll<DbContextOptions<AppDbContext>>()`.
...
- **Remediation (not yet applied):** Use `UseSqlite("DataSource=:memory:")` with a shared `SqliteConnection` instead of `UseInMemoryDatabase`. SQLite counts as a single provider replacement for SqlServer. See `.squad/decisions/inbox/creta-test-strategy.md` Â§4.
...
- `SeasonalDiscountFunction.Run()` reads `DateTime.UtcNow` directly â€” no `IClock`/`IDateTimeProvider`/`TimeProvider` abstraction.
...
- `appsettings.json` has `"AzureStorage:ConnectionString": "REPLACE_WITH_STORAGE_CONNECTION"` (placeholder). `AddBlobStorage` creates `new BlobServiceClient(connectionString)` at DI registration time â€” throws `FormatException` for placeholder.
- Fix: `builder.UseSetting("AzureStorage:ConnectionString", "")` in TestWebAppFactory â€” empty string falls back to `"UseDevelopmentStorage=true"` (valid Azurite format). Also mock `IBlobStorageService` as belt-and-suspenders.

**Coverage numbers (as of this sprint):**

- API unit tests: 45 passed, 13 skipped (integration) â€” 6 controller files Ã— avg 7 tests
- Function tests: 17 passed, 4 skipped (seasonal) â€” 3 function files Ã— avg 6 tests
- ReportsController and service-layer unit tests are out of scope for this sprint.

### 2026-05-23 â€” SQLite in-memory fix for WebApplicationFactory integration tests
...
1. **Blank the connection string early** via `builder.UseSetting("ConnectionStrings:DefaultConnection", "")` â€” this runs before `Program.cs` reads its config from `WebApplicationBuilder`.
2. **Guard `AddDatabase`** in `ServiceCollectionExtensions.cs`: skip `UseSqlServer` when the connection string is empty/null.
3. **Register SQLite** in `ConfigureServices` callback â€” now the only provider, no conflict.
4. **Move seeding to `CreateHost` override** â€” runs after the full host (and all Identity services) is built, so `UserManager`, `RoleManager` etc. are available.

**Code pattern:**

```csharp
// ServiceCollectionExtensions.cs â€” null guard prevents SQL Server in tests
...
// TestWebAppFactory.cs â€” key excerpts
...
protected override IHost CreateHost(IHostBuilder builder)
{
    ...
    db.Database.EnsureCreated();  // builds schema from EF model â€” NOT Migrate()
...
- `AddJwtAuthentication`: added `options.MapInboundClaims = false` â€” the JWT middleware's default claim mapping was converting `sub` â†’ `ClaimTypes.NameIdentifier`, silently breaking `User.FindFirstValue(JwtRegisteredClaimNames.Sub)` in `AuthController.Me()` and `Logout()`.

**Coverage numbers (after this sprint):**

- API tests: 58 passed, 0 skipped, 0 failed (45 unit + 13 integration)
- Function tests: 16 passed, 4 skipped (seasonal date-injection gap â€” unrelated to EF Core)
- Total: 74 passed, 4 skipped, 0 failed
### 2026-05-24 â€” Auth Fix Verification (role seeding deployed)

**E2E score: 12/12 âœ… (was 6/12)**
...
- Register response includes `accessToken` immediately â€” no second login required after signup.
- `GET /api/health` â†’ 200 `{"status":"ok"}` â€” health endpoint is now live (was 404 previously).
- **Orders list response is paginated** â€” `GET /api/v1/Orders` returns `{items, pageNumber, pageSize, totalCount, totalPages}`, NOT a plain array. Frontend and test code must access `.items[]` for the order list.
- All 6 previously-blocked steps (4, 5, 9, 10, 11, 12) now pass. No regressions on passing steps (1â€“3, 6â€“8).
- Updated SKILL.md known issues: health endpoint and role seeding are both resolved.

---

### 2026-05-24 â€” SameSite + JWT given_name Fix Verification (commit 22e971e)

**All 6 verification tests: âœ… PASS**
...
- `/auth/refresh` (Test 3): 200 OK â€” rotated cookie also carries `samesite=none; secure`.
- Cross-origin refresh (Test 6): `POST /refresh` with `Origin: https://brave-beach-044d7c610.6.azurestaticapps.net` â†’ 200 OK.

**JWT given_name fix confirmed:**
- `"given_name": "Creta Test"` in JWT payload â€” name, not email.
...
- After `POST /login`, the register-issued refresh token is invalidated. First refresh attempt with old cookie returned 401. After using the login cookie, refresh returned 200. Expected behavior â€” not a bug.

**API hostname clarification:**
- The provided mission URL `outdoors-shop-api-dev.azurewebsites.net` does NOT resolve (DNS failure).
- Working URL remains `app-outdoors-api-dev.azurewebsites.net` â€” same as previous sessions.

---

### 2026-05-24 â€” CORS Verification after Cinnamon-5 fix (commit cada3b2)

## 2026-05-24 â€” CORS Verification (creta-5)
...
- Verdict: PASSED â€” Cinnamon's fix is solid

**All 8 tests: âœ… PASS**

**CORS preflight confirmed (all auth + product endpoints):**
- OPTIONS /api/v1/auth/register, /auth/login, /api/v1/products with `Origin: https://brave-beach-044d7c610.6.azurestaticapps.net` â†’ 204 with correct `Access-Control-Allow-Origin` + `Access-Control-Allow-Credentials: true`
...
- All three `Set-Cookie: refreshToken` responses carry `secure; samesite=none; httponly` âœ…
...
- Login â†’ rotates register cookie; Refresh â†’ rotates login cookie. Each refresh token value is distinct.

**Negative tests confirmed (origin blocklist working):**
- `stoutdoorswebdev.z1.web.core.windows.net` (stale blob) â†’ no ACAO header (correctly rejected)
- `evil.example.com` (unknown) â†’ no ACAO header (correctly rejected)
- `wonderful-plant-0a1ca5f0f.7.azurestaticapps.net` (rogue Azure platform entry, now cleared) â†’ no ACAO header (correctly rejected)
...
## 2026-05-23 â€” Integration tests fixed (Creta)
Key learnings:
...
---
### 2026-05-25T22:39:03Z â€” Change-password feature
